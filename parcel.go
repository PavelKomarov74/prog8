...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if status != ParcelStatusRegistered {
		return fmt.Errorf("cannot update address, parcel status is not 'registered'")
	}
...
func (s ParcelStore) Delete(number int) error {
	...
	if status != ParcelStatusRegistered {
		return fmt.Errorf("cannot delete parcel with status '%s', it must be 'registered'", status)
	}
